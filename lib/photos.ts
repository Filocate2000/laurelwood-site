// lib/photos.ts
//
// Typed photo manifest. Every entry below was produced by the photo pipeline
// (scripts/process-photos.mjs: sharp, max 2400px wide, q82, metadata stripped,
// clean kebab-case output to public/images/). Each source image was inspected
// by eye and categorized + captioned accordingly.
//
// Many archival images carry an intentional "MISRAJE REAL ESTATE PARTNERS"
// watermark; that is by design, left as-is.
//
// Pages reference images by `id` via photo(id) for deterministic placement, or
// by category via byCategory(). heroFor()/bestWideHero() drive the hero slots.

export type PhotoPage =
  | "home"
  | "west-laurelwood"
  | "east-laurelwood"
  | "dona-streets"
  | "history"
  | "development-history"
  | "land-acquisition"
  | "homeowners"
  | "about"
  | "what-we-do"
  | "contact"
  | "buying"
  | "selling";

export type PhotoSection = "hero" | "feature" | "gallery" | "team";

export type PhotoCategory =
  | "archival-ads"
  | "documents"
  | "street-signs"
  | "period-photos"
  | "renderings"
  | "highway-shields"
  | "vista"
  | "stock";

export type Photo = {
  /** Stable id (the output slug, no path/extension). Used by photo(id). */
  id: string;
  /** Public path under /images. */
  src: string;
  /** Accessibility alt text. */
  alt: string;
  /** Italic caption shown beneath framed artifacts (date where known). */
  caption?: string;
  category: PhotoCategory;
  /** Primary page placement (inference + default). Pages may reference any id. */
  page: PhotoPage | null;
  section: PhotoSection;
  width: number;
  height: number;
  /** True for landscape shots suitable for a full-bleed hero. */
  wide?: boolean;
};

export const PHOTOS: Photo[] = [
  // --- Modern vista (hero) -------------------------------------------------
  {
    id: "laurelwood-vista",
    src: "/images/laurelwood-vista.jpg",
    alt: "Present-day view over the hillside homes and greenery of Laurelwood in Studio City.",
    caption: "Laurelwood today, from the surrounding hills.",
    category: "vista",
    page: "west-laurelwood",
    section: "hero",
    width: 2400,
    height: 1596,
    wide: true,
  },
  {
    id: "dona-streets-aerial-hero",
    src: "/images/dona-streets-aerial-hero.jpg",
    alt: "Aerial view over the hillside homes and tree-lined streets of the Doña streets in Laurelwood, Studio City.",
    category: "vista",
    page: "dona-streets",
    section: "hero",
    width: 2400,
    height: 1807,
    wide: true,
  },
  {
    id: "telephone-dialing",
    src: "/images/telephone-dialing.jpg",
    alt: "Hands lifting the handset and dialing a desk telephone.",
    category: "stock",
    page: "homeowners",
    section: "hero",
    width: 2400,
    height: 1602,
    wide: true,
  },
  {
    id: "neighborhood-watch-sign",
    src: "/images/neighborhood-watch-sign.jpg",
    alt: "A Welcome Neighborhood Watch Program in Force sign against a clear blue sky.",
    category: "stock",
    page: "homeowners",
    section: "feature",
    width: 2300,
    height: 3000,
  },
  {
    id: "neighborhood-watch-hero",
    src: "/images/neighborhood-watch-hero.jpg",
    alt: "Panoramic view over the Laurelwood hillside homes and the San Fernando Valley.",
    category: "vista",
    page: "homeowners",
    section: "hero",
    width: 2400,
    height: 1250,
    wide: true,
  },

  // --- Archival newspaper ads (1958-1966) ----------------------------------
  {
    id: "april-13-1966-east-laurelwood-ad",
    src: "/images/april-13-1966-east-laurelwood-ad.png",
    alt: "Laurelwood Realty Co. classified newspaper ad from April 13, 1966.",
    caption: "Laurelwood Realty Co. listing ad, April 13, 1966.",
    category: "archival-ads",
    page: "west-laurelwood",
    section: "gallery",
    width: 302,
    height: 418,
  },
  {
    id: "cannell-chaffin-mother-in-law-ad",
    src: "/images/cannell-chaffin-mother-in-law-ad.png",
    alt: "Cannell & Chaffin newspaper ad for Laurelwood with the line about a happy mother-in-law.",
    caption: 'Cannell & Chaffin campaign ad, "even my mother-in-law\'s happy."',
    category: "archival-ads",
    page: "west-laurelwood",
    section: "gallery",
    width: 1150,
    height: 416,
  },
  {
    id: "cannell-chaffin-sensations-not-words-ad",
    src: "/images/cannell-chaffin-sensations-not-words-ad.png",
    alt: 'Cannell & Chaffin model-home ad for Laurelwood, "described in sensations, not words."',
    caption: 'Cannell & Chaffin model-home ad, "sensations, not words."',
    category: "archival-ads",
    page: "west-laurelwood",
    section: "gallery",
    width: 396,
    height: 292,
  },
  {
    id: "bel-air-of-the-valley-ad",
    src: "/images/bel-air-of-the-valley-ad.png",
    alt: 'East Laurelwood newspaper ad billing it "The Bel-Air of the Valley," new custom homes from $39,950.',
    caption: 'East Laurelwood, "The Bel-Air of the Valley," new custom homes from $39,950.',
    category: "archival-ads",
    page: "east-laurelwood",
    section: "gallery",
    width: 662,
    height: 762,
  },
  {
    // Full, uncropped scan of the same Gateway Homes / Bel-Air ad: includes the
    // "Gateway Homes, Inc. at LAURELWOOD" masthead and the $39,950 price/terms
    // block that the cropped bel-air-of-the-valley-ad.png is missing. Used on
    // the west-laurelwood Fryman Road band.
    id: "gateway-belair-ad-full",
    src: "/images/gateway-belair-ad-full.jpg",
    alt: 'Full Gateway Homes newspaper ad for Laurelwood, "The Bel-Air of the Valley," custom homes from $39,950.',
    caption: 'Laurelwood advertised as the Bel-Air of the Valley.',
    category: "archival-ads",
    page: "west-laurelwood",
    section: "gallery",
    width: 1158,
    height: 2189,
  },

  // --- Origins: billboard photo + renderings + recorded map ----------------
  {
    id: "gateway-homes-billboard",
    src: "/images/gateway-homes-billboard.jpg",
    alt: 'Two men at the "Gateway Homes Inc. at Laurelwood" roadside billboard on the future tract.',
    caption:
      "Henry L. Gatz, president of Gateway Homes, announces the construction of 20 new homes in Laurelwood, with Joe Staller of the Southern California Gas Company observing. Studio City, November 27, 1960.",
    category: "period-photos",
    page: "west-laurelwood",
    section: "feature",
    width: 1102,
    height: 1226,
  },
  {
    id: "plan-4bc-renderings",
    src: "/images/plan-4bc-renderings.png",
    alt: "Architectural renderings of Laurelwood home elevations Plan 4B and Plan 4C.",
    caption: "Model elevations: Plan 4B and Plan 4C.",
    category: "renderings",
    page: "west-laurelwood",
    section: "feature",
    width: 1098,
    height: 448,
  },
  {
    id: "tract-24676-map",
    src: "/images/tract-24676-map.png",
    alt: "The recorded subdivision document for Tract No. 24676, West Laurelwood.",
    caption: "Tract No. 24676, the recorded subdivision map (1958).",
    category: "documents",
    page: "west-laurelwood",
    section: "feature",
    width: 496,
    height: 702,
  },

  // --- Street sign ---------------------------------------------------------
  {
    id: "dona-maria-street-sign",
    src: "/images/dona-maria-street-sign.png",
    alt: "Doña Maria Dr. street sign, 3100 N, with the Spanish tilde restored.",
    caption: "Doña Maria Dr., one of Laurelwood's Spanish-named streets.",
    category: "street-signs",
    page: "dona-streets",
    section: "feature",
    width: 808,
    height: 222,
    wide: true,
  },

  // --- Period life ---------------------------------------------------------
  {
    id: "neighborhood-children",
    src: "/images/neighborhood-children.png",
    alt: "Laurelwood children lined up outdoors in the early 1970s, waiting for the school bus.",
    caption: "Kids in the early 70s are lined up in Laurelwood, waiting for the school bus to arrive.",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 696,
    height: 472,
  },
  {
    id: "school-bus",
    src: "/images/school-bus.png",
    alt: "A Laurelwood child looking out the window of a Los Angeles City Schools bus in the early 1970s.",
    caption: "Laurelwood kids in the early 70s on the neighborhood bus to school.",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 696,
    height: 472,
  },

  // --- Documents: freeway study map ----------------------------------------
  {
    id: "route-170-freeway-study-map-1970",
    src: "/images/route-170-freeway-study-map-1970.png",
    alt: "Study zone map for the proposed Route 170 (Laurel Canyon) Freeway.",
    caption: "Study zone map, the proposed Route 170 (Laurel Canyon) Freeway, February 6, 1970.",
    category: "documents",
    page: "west-laurelwood",
    section: "feature",
    width: 1534,
    height: 876,
  },

  // --- Highway shields -----------------------------------------------------
  {
    id: "ca-170-shield",
    src: "/images/ca-170-shield.png",
    alt: "California State Route 170 highway shield.",
    caption: "California State Route 170.",
    category: "highway-shields",
    page: "west-laurelwood",
    section: "gallery",
    width: 552,
    height: 492,
  },
  {
    id: "ca-90-shield",
    src: "/images/ca-90-shield.png",
    alt: "California State Route 90 highway shield.",
    caption: "California State Route 90.",
    category: "highway-shields",
    page: "west-laurelwood",
    section: "gallery",
    width: 466,
    height: 486,
  },

  // --- East Laurelwood: hero, 1964 ads, present-day home photos ------------
  {
    id: "east-laurelwood-hero",
    src: "/images/east-laurelwood-hero.jpg",
    alt: "Scenic view of East Laurelwood neighborhood and The Doña Streets in Studio City, featuring mid-century homes and lush landscapes",
    category: "vista",
    page: "east-laurelwood",
    section: "hero",
    width: 1905,
    height: 755,
    wide: true,
  },
  {
    // Aug 23, 1964 ad. Single scan that carries BOTH the "Nature's Penthouse
    // in the Sky" headline AND the "$47,500-$66,000 / Louise Rogers Realty"
    // block — i.e. the task's "Nature's Penthouse" and "Louise Rogers" ads are
    // the same image. Caption from the extraction file.
    id: "aug-23-1964-laurelwood-ad",
    src: "/images/aug-23-1964-laurelwood-ad.jpg",
    alt: 'August 23, 1964 Laurelwood Estates ad, "Nature\'s Penthouse in the Sky," $47,500-$66,000, Louise Rogers Realty.',
    caption: "Vintage August 23, 1964 real estate ad promoting new home purchases in Laurelwood, Studio City",
    category: "archival-ads",
    page: "east-laurelwood",
    section: "gallery",
    width: 369,
    height: 474,
  },
  {
    // Aug 16, 1964 ad. Single scan that carries BOTH the "ONLY ONCE in a
    // LIFETIME" free-car offer AND the "FREE!! 9 NEW HOMES" Doña-streets
    // address list — i.e. the task's "free-car" and "9 NEW HOMES" ads are the
    // same image. Caption from the extraction file.
    id: "aug-16-1964-free-car-ad",
    src: "/images/aug-16-1964-free-car-ad.jpg",
    alt: 'August 16, 1964 Laurelwood Estates ad, "ONLY ONCE in a LIFETIME," free 1965 compact car, FREE!! 9 NEW HOMES on the Doña streets.',
    caption: "August 16, 1964 Laurelwood real estate ad offering a free compact car with home purchase",
    category: "archival-ads",
    page: "east-laurelwood",
    section: "gallery",
    width: 175,
    height: 474,
  },
  {
    id: "east-laurelwood-street-view",
    src: "/images/east-laurelwood-street-view.jpg",
    alt: "East Laurelwood Home",
    caption: "East Laurelwood Home",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 527,
    height: 395,
  },
  {
    id: "east-laurelwood-traditional-home",
    src: "/images/east-laurelwood-traditional-home.jpg",
    alt: "Charming Laurelwood home with classic architectural details and spacious front yard",
    caption: "Charming Laurelwood home with classic architectural details and spacious front yard",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 527,
    height: 403,
  },
  {
    id: "east-laurelwood-hillside-home",
    src: "/images/east-laurelwood-hillside-home.jpg",
    alt: "Beautiful Laurelwood home with lush landscaping and a well-maintained exterior",
    caption: "Beautiful Laurelwood home with lush landscaping and a well-maintained exterior",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 381,
    height: 311,
  },
  {
    id: "east-laurelwood-midcentury-home",
    src: "/images/east-laurelwood-midcentury-home.jpg",
    alt: "Classic mid-century modern home in Laurelwood, showcasing iconic architectural design",
    caption: "Classic mid-century modern home in Laurelwood, showcasing iconic architectural design",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 381,
    height: 311,
  },
  {
    id: "east-laurelwood-contemporary-home",
    src: "/images/east-laurelwood-contemporary-home.jpg",
    alt: "Contemporary modern home in Laurelwood, featuring sleek lines and open spaces",
    caption: "Contemporary modern home in Laurelwood, featuring sleek lines and open spaces",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 527,
    height: 394,
  },
  {
    id: "east-laurelwood-modern-home",
    src: "/images/east-laurelwood-modern-home.jpg",
    alt: "Modern Laurelwood home with minimalist design.",
    caption: "Modern Laurelwood home with minimalist design.",
    category: "period-photos",
    page: "east-laurelwood",
    section: "gallery",
    width: 381,
    height: 311,
  },

  // --- Development History: 1958 brochure strips, scenic shots, exterior plans -
  {
    id: "laurelwood-streets-1958-hero",
    src: "/images/laurelwood-streets-1958-hero.jpg",
    alt: "Vintage photo of Laurelwood streets, showcasing early development in Studio City's Laurelwood and The Dona Streets",
    category: "period-photos",
    page: "development-history",
    section: "hero",
    width: 2400,
    height: 1443,
    wide: true,
  },
  {
    id: "laurelwood-floorplan-strip-1",
    src: "/images/laurelwood-floorplan-strip-1.jpg",
    alt: "Original floorplan 1 from Laurelwood land development brochure, showcasing mid-century home design",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1460,
  },
  {
    id: "laurelwood-floorplan-strip-2",
    src: "/images/laurelwood-floorplan-strip-2.jpg",
    alt: "Original floorplan 2 from Laurelwood land development brochure, showcasing mid-century home design",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1448,
  },
  {
    id: "laurelwood-floorplan-strip-3",
    src: "/images/laurelwood-floorplan-strip-3.jpg",
    alt: "Original floorplan 3 from Laurelwood land development brochure, showcasing mid-century home design",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1455,
  },
  {
    id: "laurelwood-floorplan-strip-4",
    src: "/images/laurelwood-floorplan-strip-4.jpg",
    alt: "Original floorplan 4 from Laurelwood land development brochure, showcasing mid-century home design",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1432,
  },
  {
    id: "laurelwood-scenic-1",
    src: "/images/laurelwood-scenic-1.jpg",
    alt: "Panoramic view of Laurelwood neighborhood in Studio City, highlighting natural landscapes",
    category: "vista",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1800,
  },
  {
    id: "laurelwood-scenic-2",
    src: "/images/laurelwood-scenic-2.jpg",
    alt: "Scenic vista of The Dona Streets in Studio City, capturing the beauty of Laurelwood's hills",
    category: "vista",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1800,
  },
  {
    id: "laurelwood-scenic-3",
    src: "/images/laurelwood-scenic-3.jpg",
    alt: "Laurelwood mountain views with lush greenery, showcasing the charm of Studio City",
    category: "vista",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1800,
  },
  {
    id: "laurelwood-scenic-4",
    src: "/images/laurelwood-scenic-4.jpg",
    alt: "Sunlit view over The Dona Streets and Laurelwood, featuring iconic mid-century architecture",
    category: "vista",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 1800,
  },
  {
    id: "laurelwood-exterior-plan-1",
    src: "/images/laurelwood-exterior-plan-1.jpg",
    alt: "Floor plan number 1 from the Laurelwood land development brochure, Studio City",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 923,
  },
  {
    id: "laurelwood-exterior-plan-2",
    src: "/images/laurelwood-exterior-plan-2.jpg",
    alt: "Front exterior of a mid-century home plan in Laurelwood, Studio City",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 946,
  },
  {
    id: "laurelwood-exterior-plan-3",
    src: "/images/laurelwood-exterior-plan-3.jpg",
    alt: "Architectural home plan showcasing a modern facade in The Dona Streets, Studio City",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 951,
  },
  {
    id: "laurelwood-exterior-plan-4",
    src: "/images/laurelwood-exterior-plan-4.jpg",
    alt: "Exterior design of a spacious Laurelwood home with lush landscaping",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 944,
  },
  {
    id: "laurelwood-interior-rendering",
    src: "/images/laurelwood-interior-rendering.jpg",
    alt: "Vintage rendering of a mid-century living room from the original Laurelwood development brochure",
    category: "renderings",
    page: "development-history",
    section: "gallery",
    width: 2400,
    height: 2006,
  },
  {
    // Intro "two figures" band on /development-history. Same archival portrait
    // as harry-c-fryman-portrait (land-acquisition), registered here under its
    // own id for the development-history intro plate. Carries the intentional
    // MISRAJE REAL ESTATE PARTNERS watermark, left as-is.
    id: "harry-c-fryman",
    src: "/images/harry-c-fryman.jpg",
    alt: "Harry C. Fryman, the developer and landowner whose heirs first held the Laurelwood property, photographed among ornate furnishings in a bungalow atop a hotel he owned.",
    category: "period-photos",
    page: "development-history",
    section: "feature",
    width: 1409,
    height: 1279,
  },

  // --- Land Acquisition History: mountains hero, Fryman portrait, entrance sign
  {
    id: "laurelwood-mountains-hero",
    src: "/images/laurelwood-mountains-hero.jpg",
    alt: "Historical view of the Laurelwood mountains in Studio City, capturing the early land acquisition for Laurelwood and The Dona Streets",
    category: "vista",
    page: "land-acquisition",
    section: "hero",
    width: 2400,
    height: 941,
    wide: true,
  },
  {
    id: "harry-c-fryman-portrait",
    src: "/images/harry-c-fryman-portrait.jpg",
    alt: "Harry C. Fryman, a prominent developer, is pictured in a bungalow located on the top floor of a hotel he owned.",
    category: "period-photos",
    page: "land-acquisition",
    section: "feature",
    width: 1409,
    height: 1279,
  },
  {
    id: "laurelwood-entrance-sign",
    src: "/images/laurelwood-entrance-sign.jpg",
    alt: "Laurelwood neighborhood entrance sign in Studio City, welcoming residents and visitors",
    category: "period-photos",
    page: "land-acquisition",
    section: "gallery",
    width: 1240,
    height: 450,
  },
];

// Self-hosted video (copied verbatim by the pipeline to public/video/). The
// caption lives with the page copy in content/west.ts; this is a fallback.
export const VIDEOS = {
  "disorderly-orderly": {
    src: "/video/disorderly-orderly-1964.mp4",
    caption: "West Laurelwood, mid-construction, as seen in The Disorderly Orderly (1964).",
  },
} as const;

// --- Accessors --------------------------------------------------------------

/** Look up a photo by its stable id. Returns null if missing. */
export function photo(id: string): Photo | null {
  return PHOTOS.find((p) => p.id === id) ?? null;
}

/** All photos in a category, in manifest order. */
export function byCategory(category: PhotoCategory): Photo[] {
  return PHOTOS.filter((p) => p.category === category);
}

/** The hero image for a page, or null (caller falls back to the gradient). */
export function heroFor(page: PhotoPage): Photo | null {
  return PHOTOS.find((p) => p.page === page && p.section === "hero") ?? null;
}

/** All photos whose primary placement is this page. */
export function photosFor(page: PhotoPage): Photo[] {
  return PHOTOS.filter((p) => p.page === page);
}

/** The best wide landscape photo for a full-bleed hero, or null. */
export function bestWideHero(): Photo | null {
  return (
    PHOTOS.find((p) => p.section === "hero" && p.wide) ??
    PHOTOS.find((p) => p.wide) ??
    null
  );
}

/** Photos the pipeline could not confidently place (for STATE.md review). */
export function unplacedPhotos(): Photo[] {
  return PHOTOS.filter((p) => p.page === null);
}
